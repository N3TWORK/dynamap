...
`compressCollection` | no | string: `gzip`. When set Dynamap will compress the entire collection as persist it as a binary type using the compression method. Currently only `gzip` is supported. Note that as the entire collection is compressed this has the same effect as using `replace`, i.e. fine grained updates are not possible and so concurrent operations are not safe.
